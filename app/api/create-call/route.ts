...
const DEFAULT_ASSISTANT_ID = "50cedc84-8edc-493d-8995-ef6bf7c33399";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, email, phone, assistantId } = body;
    ...
    const sanitizedAssistantId =
      typeof assistantId === "string" ? assistantId.trim() : "";
    const fieldsToCheck = [name, email, phone];
    if (sanitizedAssistantId) {
      fieldsToCheck.push(sanitizedAssistantId);
    }

    if (containsUnsafeLanguageInFields(fieldsToCheck)) {
      return NextResponse.json(
        {
          error:
    ...
    const assistantIdToUse = sanitizedAssistantId || DEFAULT_ASSISTANT_ID;
